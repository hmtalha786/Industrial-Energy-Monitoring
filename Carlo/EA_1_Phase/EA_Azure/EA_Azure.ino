...
void loop() {
  ...
  delay(100);  get_Voltage();
  delay(100);  get_Current();
  delay(100);  get_True_Power();
  delay(100);  get_Power_Derivatives();
  delay(100);  get_Frequency();
  Serial.println("*********************************************************");

  // JSON Packet Sent after every 300 sec ...................................................
  if ( millis() >= timer ) { timer = millis() + 10000UL; json_packet_sender(); }
}
...
void json_packet_sender() {
  ...
  if (hasIoTHub){
    if (Esp32MQTTClient_SendEvent(JSONmessageBuffer)){Serial.println("Sending data succeed");} else {Serial.println("Failure...");}
  }
}
